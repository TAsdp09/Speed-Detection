...
def process_camera(camera_index, camera_name):
    ...
    SPEED_HISTORY = {}  # Untuk moving average
        ...
        for object_id, data in new_positions.items():
            if object_id in prev_positions:
                ...
                if label not in ["Orang"]:  # Only track vehicles
                    ...
                    if len(SPEED_HISTORY[object_id]) > 5:  # Max 5 readings
                        SPEED_HISTORY[object_id].pop(0)
                    
                    avg_speed = sum(SPEED_HISTORY[object_id]) / len(SPEED_HISTORY[object_id]) if SPEED_HISTORY[object_id] else 0.0
                    
                    # Log to CSV (use the average speed for logging and violation check)
                    violation = avg_speed > SPEED_THRESHOLD
                    log_to_csv(
                        camera_name=camera_name,
                        object_type=label,
                        speed=avg_speed,
                    ...
                    if violation:
                        ...
                        if (vehicle_id not in last_notification or 
                            ...
                            cv2.putText(notification_frame, f"SPEEDING: {avg_speed:.2f} km/h", 
                                      (x1, y1 - 40), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
                            
                            threading.Thread(
                                target=send_telegram_notification,
                                args=(camera_name, label, avg_speed, notification_frame)
                            ).start()
